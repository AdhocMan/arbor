...
TEST(iexpr, distance_locset) {
    ...
    auto dist = thingify(arb::iexpr::distance(2.0, arb::mlocation{1, 0.2}), prov);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {0, 0.0, 1.0}), 2.0 * 7.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {1, 0.0, 1.0}), 2.0 * 3.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {2, 0.0, 1.0}), 2.0 * 12.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {3, 0.0, 1.0}), 2.0 * 22.0);

    // test distance to multiple points
    dist = thingify(arb::iexpr::distance(2.0, arb::mlocation_list({arb::mlocation{1, 1.0}, arb::mlocation{2, 1.0}})), prov);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {0, 0.0, 1.0}), 2.0 * 15.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {1, 0.0, 1.0}), 2.0 * 5.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {2, 0.0, 1.0}), 2.0 * 10.0);
    EXPECT_DOUBLE_EQ(dist->eval(prov, {3, 0.0, 1.0}), 2.0 * 30.0);
}
